...
def create_router(database: Database, settings: Settings) -> Router:
    ...
    async def begin(message: Message, state: FSMContext, user_id: int | None = None) -> None:
        language = database.get_language(user_id or message.from_user.id)
        await state.set_state(FeedbackState.waiting_for_message)
        await message.answer(f"{get_text(language, 'feedback_prompt')}\n\n{get_text(language, 'privacy')}")
    ...
    async def cancel_feedback(message: Message, state: FSMContext) -> None:
        ...
        await state.clear()
        await message.answer(get_text(language, "cancelled"), reply_markup=main_menu(language, message.from_user.id in settings.admin_ids))
    ...
    async def save_feedback(message: Message, state: FSMContext, bot: Bot) -> None:
        ...
        await state.clear()
        await message.answer(get_text(language, "feedback_saved", request_id=request_id), reply_markup=main_menu(language, message.from_user.id in settings.admin_ids))
