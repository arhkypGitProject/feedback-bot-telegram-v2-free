...
def create_router(database: Database, settings: Settings) -> Router:
    ...
    async def show_admin(target: Message | CallbackQuery) -> None:
        ...
        if not is_admin(user_id):
            if isinstance(target, CallbackQuery):
                await target.answer(
                    get_text(database.get_language(user_id), "admin_only"),
                    show_alert=True,
                )
            else:
                await target.answer(
        ...
        text = (
            get_text(language, "admin_title", count=len(requests))
            if requests
            else get_text(language, "admin_empty")
        )
    ...
    @router.callback_query(
        lambda query: query.data and query.data.startswith("admin_list:")
    )
    async def admin_list(callback: CallbackQuery) -> None:
        if not is_admin(callback.from_user.id):
            await callback.answer(
                get_text(database.get_language(callback.from_user.id), "admin_only"),
                show_alert=True,
            )
            return
        ...
        title_key = {
            "open": "admin_section_open",
            "closed": "admin_section_closed",
            "all": "admin_section_all",
        }[section]
        text = (
            get_text(language, title_key, count=len(requests))
            if requests
            else get_text(language, "no_requests")
        )
        await callback.message.edit_text(
            text, reply_markup=admin_menu(requests, language)
        )
        await callback.answer()

    @router.callback_query(
        lambda query: query.data and query.data.startswith("admin_request:")
    )
    async def admin_request(callback: CallbackQuery) -> None:
        ...
        if not request:
            await callback.answer(
                get_text(database.get_language(callback.from_user.id), "not_found"),
                show_alert=True,
            )
            return
        language = database.get_language(callback.from_user.id)
        conversation = "\n".join(
            f"<b>{item['author_role']}</b> [{item['created_at']}]\n{html.escape(item['body'])}"
        ...
        await callback.message.edit_text(
            ...
            reply_markup=admin_request_menu(request_id, language, request["status"]),
        )
        await callback.answer()

    @router.callback_query(
        lambda query: query.data and query.data.startswith("admin_close:")
    )
    async def close_request(callback: CallbackQuery) -> None:
        ...
        language = database.get_language(callback.from_user.id)
        await callback.answer(
            get_text(language, "request_closed", request_id=request_id)
        )
        await callback.message.edit_reply_markup(
            reply_markup=admin_request_menu(request_id, language, "closed")
        )

    @router.callback_query(
        lambda query: query.data and query.data.startswith("admin_reopen:")
    )
    async def reopen_request(callback: CallbackQuery) -> None:
        ...
        language = database.get_language(callback.from_user.id)
        await callback.answer(
            get_text(language, "request_reopened", request_id=request_id)
        )
        await callback.message.edit_reply_markup(
            reply_markup=admin_request_menu(request_id, language, "open")
        )

    @router.callback_query(
        lambda query: query.data and query.data.startswith("admin_reply:")
    )
    async def start_reply(callback: CallbackQuery, state: FSMContext) -> None:
        ...
        await callback.message.answer(
            get_text(
                database.get_language(callback.from_user.id),
                "reply_prompt",
                request_id=request_id,
            )
        )
    ...
    async def send_reply(message: Message, state: FSMContext, bot: Bot) -> None:
        ...
        if not request or not body or len(body) > 4000:
            await message.answer(
                get_text(database.get_language(message.from_user.id), "reply_invalid")
            )
            return
        ...
        await message.answer(
            get_text(
                database.get_language(message.from_user.id),
                "reply_sent",
                request_id=request_id,
            )
        )
        ...
        try:
            ...
            logger.warning(
                "Could not deliver reply for request %s: %s", request_id, error
            )
            await message.answer(
                get_text(database.get_language(message.from_user.id), "delivery_failed")
            )
